		...
		while (max > 0) {
			...
			for (int j = 0; j < 8; j++) {
				if (toBe[j] == 0) continue;
					...
					toBe[j]--;
					while (pos[j] > 0) {
						if (current_board.board[pos[j]][j] == Void) {
							for (int k = pos[j] - 1; k >= 0; k--) {
								current_board.board[k + 1][j] = current_board.board[k][j];
								current_board.board[k][j] = Void;
							}
							if (current_board.board[pos[j]][j] != Void) pos[j]--;
							break;
						}
						pos[j]--;
					}
